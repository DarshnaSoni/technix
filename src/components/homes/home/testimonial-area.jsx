...
const TestimonialArea = () => {
	...
	useEffect(() => {
		fetch(
			"http://localhost:1337/api/landingpages?populate[TestimonialSection][populate][testimonials][populate][avatar][populate]=true&populate[TestimonialSection][populate][testimonials][populate][avatar][fields][0]=name&populate[TestimonialSection][populate][testimonials][populate][avatar][fields][1]=url"
		)
			.then((response) => response.json())
			.then((data) => {
				setTestimonialData(data.data[0].attributes.TestimonialSection[0]);
				setTestimonialCard(
					data.data[0].attributes.TestimonialSection[0].testimonials
				);
			})
			.catch((error) => {
						...
						<div className='col-lg-12'>
							<div className='tp-testimonial-title-wrapper text-center'>
								<span className='tp-section-title__pre'>
									{testimonialData.shortTitle1 + " "}
									<span className='title-pre-color'>
										{testimonialData.shortTitle2}
									</span>
									<AngleArrow />
								</span>
								<h3 className='tp-section-title'>
									{testimonialData.title1 + " "}
									<span className='title-color'>{testimonialData.title2}</span>
							...
							<div className='testimonial-active swiper-container'>
								<Swiper {...setting}>
									{testimonialCard.map((item, i) => (
										<SwiperSlide
											key={i}
											className='tp-testimonial-item text-center mb-30'>
