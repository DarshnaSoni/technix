...
const IndustryArea = ({ service }) => {
	...
	const [industryCover, setIndustryCover] = useState([]);
	const [industryDetails, setIndustryDetails] = useState([]);

	useEffect(() => {
		fetch(
			"http://localhost:1337/api/services?populate[IndustrySection][populate][industrycover][populate][cards][populate][Image][populate]=true&populate[IndustrySection][populate][industrycover][populate][cards][populate][Image][fields][0]=name&populate[IndustrySection][populate][industrycover][populate][cards][populate][Image][fields][1]=url&populate[IndustrySection][populate][industrycards][populate][Image][populate]=true&populate[IndustrySection][populate][industrycards][populate][Image][fields][0]=name&populate[IndustrySection][populate][industrycards][populate][Image][fields][1]=url&populate[IndustrySection][populate][industrycards][populate][Services][populate]=*"
		)
			.then((response) => response.json())
			.then((data) => {
				setIndustryData(data.data[0].attributes.IndustrySection[0]);
				setIndustryCover(
					data.data[0].attributes.IndustrySection[0].industrycover
				);
				setIndustryDetails(
					data.data[0].attributes.IndustrySection[0].industrycards
				);
			})
			.catch((error) => {
		...
		<>
			<section
				...
				<div className='tp-industry-shape'>
					<Image
						className='shape-1'
						src={industry_shape_1}
						alt='theme-pure'
					/>
					<Image
						className='shape-2'
						src={industry_shape_2}
						alt='theme-pure'
					/>
					<Image
						className='shape-3'
						src={industry_shape_3}
						alt='theme-pure'
					/>
				</div>
						...
						<div className='col-lg-12'>
							<div className='tp-industry-title-wrapper text-center'>
								<span className='tp-section-title__pre'>
									{industryData.shortTitle1}{" "}
									<span className='title-pre-color'>
										{industryData.shortTitle2}
									</span>
									<AngleArrow />
								</span>
							...
							<div className='tp-industry-wrapper mb-30 fadeLeft'>
								<div className='tp-industry-content'>
									<h3 className='tp-industry-content-title'>
										{industryCover.Title1} <br />
										{industryCover.Title2}
									</h3>
									<div className='tp-industry-thumb-wrapper d-flex'>
										{industryCover.cards &&
											industryCover.cards.map((item, i) => (
												<div
													className='tp-industry-thumb text-center'
													key={i}>
													<Image
											...
											industryDetails.map((item, i) => (
												<li
													...
													<button
														...
														id={`pills-${item.IDname}-tab`}
														data-bs-toggle='pill'
														data-bs-target={`#${item.IDname}`}
														type='button'
														role='tab'
														aria-controls={`${item.IDname}`}
										...
										{industryDetails &&
											industryDetails.map((item, i) => (
												<div
													...
													id={item.IDname}
													role='tabpanel'
													aria-labelledby={`pills-${item.IDname}-tab`}
													key={i}>
															...
															<div className='tp-industry__progresss'>
																...
																<div className='progress'>
																	<div
																		...
																		style={{
																			width: `${item.ScalePercentage}%`,
																		}}
																		aria-valuenow='25'
																		aria-valuemin='0'
																		aria-valuemax='100'>
																		<span>{item.ScalePercentage}%</span>
																	</div>
																</div>
															</div>
														</div>
